# 로그 항목 선정 전략: "무엇을 로그로 남겨야 하는가"

> **이 문서의 위치**
>
> | 문서 | 다루는 질문 |
> |------|-----------|
> | `observability-guide.md` | **WHY** — 왜 Observability가 필요한가, 3대 축 개념 |
> | `observability-developer-setup.md` | **HOW** — Spring Boot에서 어떻게 설정하고 구현하는가 |
> | `observability-code-separation.md` | **WHERE** — 로그/메트릭 코드를 어디에 배치하는가 (분리 패턴) |
...
> 참고: `observability-code-separation.md`의 `RequestLoggingInterceptor` 패턴이 이 로그를 담당한다.
...
AOP를 쓰면 진입/종료를 자동으로 DEBUG 처리할 수 있다 (`observability-code-separation.md` 참고).
...
**해결**: `observability-guide.md`의 패턴 2 참고 — 4xx는 WARN, 5xx는 ERROR.
