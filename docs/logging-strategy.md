...
> **속성 명명 기준**: 이 문서의 로그 필드명은 [OpenTelemetry Semantic Conventions v1.23.0+ (Stable)](https://opentelemetry.io/docs/specs/semconv/http/http-spans/)을 따른다.
> 구버전(≤v1.20)과의 매핑이 필요하면 [HTTP Migration Guide](https://opentelemetry.io/docs/specs/semconv/non-normative/http-migration/)를 참고한다.
...
**필드** (OTel Semantic Conventions 기준):

| 필드명 | OTel 속성 | Requirement Level | 설명 |
|--------|-----------|-------------------|------|
| HTTP 메서드 | `http.request.method` | Required | 요청 메서드 (`GET`, `POST` 등) |
| URI 경로 | `url.path` | Required | 요청 경로 (`/shortenUrl`, `/abc123` 등) |
| 응답 상태코드 | `http.response.status_code` | Conditionally Required | HTTP 응답 코드 (`200`, `404`, `500` 등) |
| 소요 시간 | `duration_ms` | *(커스텀 필드)* | 요청 처리 시간(ms). OTel 메트릭에서는 `http.server.request.duration`(단위: 초)을 사용하지만, 로그에서는 가독성을 위해 ms 단위 커스텀 필드를 사용한다 |

> **참고 — OTel 구버전(≤v1.20)과의 차이**:
> `http.method`→`http.request.method`, `http.status_code`→`http.response.status_code`로 변경되었다.
> `request.`/`response.` 접두어가 추가된 이유는 방향(요청/응답)을 명시하기 위함이며,
> `url.*` 네임스페이스는 HTTP 외 다른 프로토콜에서도 재사용할 수 있도록 분리되었다.
> 자세한 내용은 [OTel HTTP Stable 선언 블로그](https://opentelemetry.io/blog/2023/http-conventions-declared-stable/)를 참고한다.
...
**필드** (OTel Semantic Conventions — [Exceptions on Spans](https://opentelemetry.io/docs/specs/semconv/exceptions/exceptions-spans/) 기준):

| 필드명 | OTel 속성 | 설명 |
|--------|-----------|------|
| 예외 클래스 | `exception.type` | 예외의 FQCN (`java.lang.NullPointerException` 등) |
| 예외 메시지 | `exception.message` | 예외 메시지 텍스트 |
| 스택트레이스 | `exception.stacktrace` | 전체 스택트레이스 문자열 |
